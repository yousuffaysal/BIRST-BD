...
import { Trash2, Search, Mail, Phone, MapPin, Calendar, Clock, Building2 } from 'lucide-react';
...
const ShowEventEnrollment = () => {
    ...
    const fetchEnrollments = async () => {
        try {
            const res = await axiosSecure.get('/event-enrollments');
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900">Event Enrollments</h2>
                    <p className="text-gray-600 mt-1">Manage event registration requests</p>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Participant Info</th>
                            ...
                            {filteredEnrollments.length > 0 ? (
                                filteredEnrollments.map((enrollment) => (
                                    <tr key={enrollment._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="font-medium text-gray-900">{enrollment.eventTitle || 'Unknown Event'}</div>
                                            <div className="text-xs text-gray-500 bg-blue-50 text-blue-700 inline-block px-2 py-0.5 rounded mt-1">
                                                Event
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
